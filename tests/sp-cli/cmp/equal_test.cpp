...
class EqualComparasionTest : public testing::Test {
    protected:
...
0; JNE)";
    ...
    EqualComparasionTest(): ram(code), cpu(code) {}
};

TEST_F(EqualComparasionTest, InitialState) {
    EXPECT_EQ(ram.get(0xFF0), "0") << "Incorrect value at FF0";
    EXPECT_EQ(ram.get(0xFA0), "10") << "Incorrect value at FA0";
...
TEST_F(EqualComparasionTest, RunCode) {
    cpu.run();
