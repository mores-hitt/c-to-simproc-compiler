...
class SmallerComparasionTest : public testing::Test {
    protected:
...
0; JNE)";
    ...
    SmallerComparasionTest(): ram(code), cpu(code) {}
};

TEST_F(SmallerComparasionTest, InitialState) {
    EXPECT_EQ(ram.get(0xFA0), "10") << "Incorrect value at FF0";
    EXPECT_EQ(ram.get(0xFA1), "11") << "Incorrect value at FA0";
...
TEST_F(SmallerComparasionTest, RunCode) {
    cpu.run();
