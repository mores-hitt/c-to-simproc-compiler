...
class MediumAlmostZeroTest : public testing::Test {
    protected:
...
0001010001111011;
1100000010100100; valores a operar
0001011000011110;
...
0; JNE)";
    ...
    MediumAlmostZeroTest(): ram(code), cpu(code) {}
};

TEST_F(MediumAlmostZeroTest, InitialState) {
    EXPECT_EQ(ram.get(0xFA0), "0100000010100100") << "Incorrect value at FF0";
    EXPECT_EQ(ram.get(0xFA2), "1100000010100100") << "Incorrect value at FA2";
...
TEST_F(MediumAlmostZeroTest, RunCode) {
    cpu.run();
    sp_cli::CPUState state {cpu.getState()};

    EXPECT_EQ(state.memory.get(0xFC0), "1011100101010001") << "ADDF does not provide the correct result";
    EXPECT_EQ(state.memory.get(0xFC1), "1000000000000000") << "ADDF does not provide the correct result";

    EXPECT_EQ(state.CF.getFlag(sp_cli::Flags::Z), false) << "Z flag is set, and it should";
    EXPECT_EQ(state.CF.getFlag(sp_cli::Flags::N), true) << "N flag is not set, and it should";
    ...
    EXPECT_EQ(state.memory.get(0xFF2), "0000000000000001") << "JME did not jump, but it should have";
    EXPECT_EQ(state.memory.get(0xFF3), "0") << "JMA jumped, but it should not have";
