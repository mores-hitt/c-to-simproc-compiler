...
class MediumZeroTest : public testing::Test {
    protected:
...
0; JNE)";
    ...
    MediumZeroTest(): ram(code), cpu(code) {}
};

TEST_F(MediumZeroTest, InitialState) {
...
TEST_F(MediumZeroTest, RunCode) {
    cpu.run();
    sp_cli::CPUState state {cpu.getState()};

    EXPECT_EQ(state.memory.get(0xFC0), "0000000000000000") << "ADDF does not provide the correct result";
    EXPECT_EQ(state.memory.get(0xFC1), "0000000000000000") << "ADDF does not provide the correct result";

    EXPECT_EQ(state.CF.getFlag(sp_cli::Flags::Z), true) << "Z flag is not set, and it should";
    ...
    EXPECT_EQ(state.memory.get(0xFF1), "0000000000000001") << "JEQ did not jump, but it should have.";
    ...
    EXPECT_EQ(state.memory.get(0xFF8), "0") << "JNE jumped, but it should not have";
